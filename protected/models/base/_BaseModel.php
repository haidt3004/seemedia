...
class _BaseModel extends _MultilanguagesModel {
	...
	public $allowImageType = 'jpg, png, gif, jpeg';
	public $maxImageFileSize = 3145728; //3MB
	public $attributesBeforeSave = array(); //first time load model

	public function saveImage($fieldName, $diffModel = false) {
		$oldImage = $this->$fieldName;
		//to save image different model
		if ($diffModel == false) {
		...
		//Haidt - clean file name - replace all whitespace by underscores.
		$fileName = $this->replaceSpaceByUnderscores(urldecode($fileName));
	 ...
	 * $params string $str
	 * @return string
	 * @Todo: replace a space by underscores
	 */
	protected function replaceSpaceByUnderscores($str) {
    ...
    // //danh cho translate
    // public function getDataValidate($modelName,$language='en'){
    ...
    // public function saveDataWithLanguage($modelName){
    //     $languages = Languages::getAlllanguage();
    //     //deleteAllLanguage
    //     $this->deleteAllDataTranslate();
    //     if(is_array($languages)&&count($languages)>0){
    //         foreach($languages as $lang){
    //             $dataAttributes = $this->getDataValidate($modelName,$lang->code);
    ...
    // public function getDataWithLangauge($model,$language){
    //     $mTranslate = $this->getModelTranslate();
    ...
    //             $model->attributes = $model->getDataValidate($this->getModelName($model),$language);
    //             return $model;
    //         }
    //     }
    //     return $model;
