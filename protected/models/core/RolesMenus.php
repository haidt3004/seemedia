...
class RolesMenus extends _BaseModel
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('id, role_id, menu_id,actions,role_website_id', 'safe'),
		);
	}
    ...
    public function uniquePair($attribute,$params){
        if(!$this->hasErrors())
        {
            $rolesMenus = RolesMenus::model()->find('role_website_id = '.ROLE_WEBSITE_ID.' and t.menu_id ='.$this->menu_id.' AND t.role_id = '.$this->role_id);
            if(!empty($rolesMenus))
