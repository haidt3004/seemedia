...
//          $patternDate = '/([0-9]{1})\/([0-9]{1})\/([0-9]{4})/';
	...
	//Vnguyen
	public static function timeAgo($time = 0) {
		$diff = time() - $time;
	 ...
	 * @param string $string -> Y-m-d H:i:s
	 * @return string  ->  j F Y  : <!--20 January 2014-->
	 * @Todo: format date
	 */
	public static function toDateTimeForDisplay($string) {
		$date = date('d M Y', strtotime($string));
		return $date;
	}
	
	/**
	 * Hiep date format multilang
	 */
	public static function toDateMultiLang($string_date)
	{
		$month = date_format(date_create($string_date), "F");
		$day = date_format(date_create($string_date), "j");
		$year = date_format(date_create($string_date), "Y");
		// multilang
		$month = TextMultilangHelper::label(strtolower($month));
