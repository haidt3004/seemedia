...
class Paypal {
	 ...
	 *
	 */
	public static function send($aItems, $return_url, $cancel_url, $notify_url, $currencyCode = 'SGD') {

		if ($currencyCode == 'MYR') {
			$paypal_email = Yii::app()->params['paypalBusinessEmailMY']; //bussiness email
			$paypal_type = Yii::app()->params['paypalTypeMY']; //live or test
		}else{
			$paypal_email = Yii::app()->params['paypalBusinessEmailSG']; //bussiness email
			$paypal_type = Yii::app()->params['paypalTypeSG']; //live or test
		}
	...
	public static function isValid() {
		...
		if (Yii::app()->params['paypalTypeMY'] == 'test' || Yii::app()->params['paypalTypeSG'] = 'test')
