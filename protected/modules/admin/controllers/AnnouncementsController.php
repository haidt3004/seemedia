...
class AnnouncementsController extends AdminController {
    ...
    public function actionCreate() {
        try {
            ...
            if (isset($_POST['Announcement'])) {
                ...
                $model->status = $_POST['Announcement']['status'];
                $model->parent_id = $_POST['Announcement']['parent_id'];
                if ($model->save()) {
                    $model->saveDataWithLanguage('Announcement');
    ...
    public function actionDeleteAll() {
        ...
        if (!empty($shouldDelete)) {
            $deleteImages = Announcement::model()->findAll('id in (' . implode(',', $shouldDelete) . ')');
...
//                    PostsCategories::model()->deleteAllByCategory($item->id);
            ...
            Announcement::model()->deleteAll('id in (' . implode(',', $shouldDelete) . ')');
    ...
    public function actionUpdateStatusAll() {
        ...
        if (!empty($updateItems) && $status != '') {
            Announcement::model()->updateAll(array('status' => $status), 'id in (' . implode(',', $updateItems) . ')');
