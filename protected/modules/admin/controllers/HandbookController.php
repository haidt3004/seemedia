...
class HandbookController extends AdminController {
    public $pluralTitle = 'Handbooks';
    public $singleTitle = 'Handbook';
    public $cannotDetele = array();

    public function actionCreate() {
        try {
            $model = new Handbook('create');
            $model->display_order = $model->nextOrderNumber();
            if (isset($_POST['Handbook'])) {
                $model->attributes = $_POST['Handbook'];
    ...
    public function actionIndex() {
        try {
            $model = new Handbook('search');
            $model->unsetAttributes();  // clear any default values
            if (isset($_GET['Handbook']))
                $model->attributes = $_GET['Handbook'];
    ...
    public function actionDeleteAll() {
        ...
        if (!empty($shouldDelete)) {
            $deleteImages = Handbook::model()->findAll('id in (' . implode(',', $shouldDelete) . ')');
            ...
            Handbook::model()->deleteAll('id in (' . implode(',', $shouldDelete) . ')');
    ...
    public function actionUpdateStatusAll() {
        ...
        if (!empty($updateItems) && $status != '') {
            Handbook::model()->updateAll(array('status' => $status), 'id in (' . implode(',', $updateItems) . ')');
