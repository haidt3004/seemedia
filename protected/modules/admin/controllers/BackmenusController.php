...
class BackmenusController extends AdminController
{
	...
	public function actionCreate()
	{
		...
		$selectedRoles =  RolesMenus::model()->findAll('role_website_id ='.ROLE_WEBSITE_ID.' AND menu_id="' . $model->id . '"');
		...
		if (isset($_POST['Menus']))
		{
			...
			if ($model->save())
			{
				RolesMenus::model()->deleteAll('role_website_id ='.ROLE_WEBSITE_ID.' AND menu_id="' . $model->id . '"');
				if (!empty($allowRoles))
				{
					foreach ($allowRoles as $role)
					{
						...
						$rolesMenus->role_website_id = ROLE_WEBSITE_ID;
						$rolesMenus->save();
					}
				}
	...
	public function actionDelete($id)
	{
		...
		{
			RolesMenus::model()->deleteAll(array('condition' => 'menu_id = ' . $id . ' AND role_website_id = '.ROLE_WEBSITE_ID));
