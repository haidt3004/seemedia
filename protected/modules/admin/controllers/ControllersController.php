...
class ControllersController extends AdminController
{
    ...
    public function actionCreate()
    {
        ...
        if (isset($_POST['Controllers']))
        {
            ...
            if ($model->save())
            {
                ...
                {
                    if(!isset($_POST['allowaction'][$role->id])){
                        continue;
                    }
                    $allowActions = $_POST['allowaction'][$role->id];
                    $existActionRole = ActionsRoles::model()->find('role_website_id = '.ROLE_WEBSITE_ID.' AND controller_id = ' . (int)$model->id . " AND roles_id = " . (int)$role->id . " AND can_access = 'allow'");
                    if ($existActionRole)
                    {
                        $existActionRole->actions = implode(',', $allowActions);
                        $newActionRole->role_website_id = ROLE_WEBSITE_ID;
                        $existActionRole->update(array('actions','role_website_id'));
                    }
                    else
                    {
            ...
            if ($model->save())
            {
                ...
                foreach($listRole as $role)
                {
                    if(!isset($_POST['allowaction'][$role->id])){
                        ...
                        $existActionRole = ActionsRoles::model()->find('role_website_id = '.ROLE_WEBSITE_ID.' AND  controller_id = ' . (int)$model->id . " AND roles_id = " . (int)$role->id);
                        if($existActionRole){
                            $existActionRole->delete();
                             // die($role->id);
                            // $existActionRole->actions = null;
                            // $existActionRole->can_access = 'deny';
                            // $existActionRole->update(array('can_access'));
                        }
                        continue;
                    }
                    $allowActions    = (isset($_POST['allowaction'][$role->id])) ? $_POST['allowaction'][$role->id] : array();
                    $existActionRole = ActionsRoles::model()->find('role_website_id = '.ROLE_WEBSITE_ID.' AND controller_id = ' . (int)$model->id . " AND roles_id = " . (int)$role->id . " AND can_access = 'allow'");
                    if ($existActionRole)
                    {
                            $existActionRole->actions       = implode(',', $allowActions);
                            $existActionRole->role_website_id = ROLE_WEBSITE_ID;
                            $existActionRole->update(array('actions','role_website_id'));
                    }
                    else
                    {
    ...
    public function actionGetAvailableAction()
    {
        ...
        if ($controller != '' && $module != '')
        {
                ...
                $listRole = Roles::model()->getRoles();
                if ($contorllerObj)
                        $availableActions = ActionsRoles::model()->findAll('role_website_id = '.ROLE_WEBSITE_ID.' AND controller_id = ' . (int)$contorllerObj->id . ' AND can_access = \'allow\'');
