...
<div class="form">
    ...
    <ul class="nav nav-tabs">
        <?php
        $i = 1;
        if (SettingHrForm::$settingDefine && is_array(SettingHrForm::$settingDefine)):
            foreach (SettingHrForm::$settingDefine as $key => $item):
                            ...
                            <div class="column">
                                ...
                                if ($itemObject->items && is_array($itemObject->items)):
                                    foreach ($itemObject->items as $data):
                                        $dataObj = (object) $data;
                                            if ($key == "redemptioncode") { ?>
                                                    <?php 
                                                    if ($dataObj->name == "code_1" || $dataObj->name == "code_2" || $dataObj->name == "code_3") {
                                                        echo '<div class="form-group form-group-sm">';
                                                        echo $form->labelEx($model, $dataObj->name, array('class' => "col-sm-1 control-label")); 
                                                    }
                                                    ?>
                                                    <?php
                                                    $unit = '';
                                                    ...
                                                    elseif ($dataObj->controlTyle == 'checkbox'):
                                                        echo '<div class="col-sm-3">' . $form->checkbox($model, $dataObj->name, array_merge($dataObj->htmlOptions, array('class' => ""))) . $unit . $note . '</div>';

                                                    endif;
                                                    ?>
                                                <?php echo $form->error($model, $dataObj->name); ?>
                                                <?php 
                                                if ($dataObj->name == "plan_code_1" || $dataObj->name == "plan_code_2" || $dataObj->name == "plan_code_3") {
                                                    echo '</div>';
                                                }
                                                ?>
                                            <?php } else { ?>
