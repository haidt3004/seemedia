...
class CCodeGenerator extends CController {
	...
	public function actionIndex() {
		...
		if ($model->files != array() && isset($_POST['generate'], $_POST['answers'])) {
			...
			//haidt - add permission action
			if ($model->status == CCodeModel::STATUS_SUCCESS) {
				$currClass = get_class($model);
				if ($currClass == "ControlleradminCode") {
					//haidt - do add permission action
					$model->addPermissionAction();
					//haidt - do add back menu
					$model->addBackMenu();
				}
			}
		}
