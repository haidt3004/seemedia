...
function runEditorBasic(url, toolbar, width, height) {
	jQuery('.ver_editor_basic').each(function () {
		console.log("sdfkldsakflsalkfdslka");
...
//							'yearRange': '1960',
				...
				jQuery.datepicker.regional['en-GB'],
						{
							...
							'changeYear': 'true',
							'showOn': 'button',
							'buttonImage': url + '/images/icon_calendar_r.gif',
							'buttonImageOnly': true
						}
				)
				);
	...
	jQuery('.ver_datepicker_FE').each(function () {
		var id = jQuery(this).attr('id');
		jQuery('#' + id).datepicker(
			jQuery.extend(
				{showMonthAfterYear: false},
...
/**
 * @Author: ANH DUNG Aug 19, 2015
 * @Todo: fix admin target blank for action update
 */
function fixTargetBlank() {
//	 client complain about this feature (@.@)
//	$('.button-column a.update').attr('target', '_blank');
